...
The $n$-particle distribution function is defined via the $n$-particle density as follows (for reference, see Section~2.6 in~\cite{HansenMcDonald13})
\begin{equation*}
	\label{def:g_n}
...
\begin{eqnarray*}
	\label{def:rho_n}
	\rho^{(n)}({\vb r}_1, \dotsc ,{\vb r}_n) = \frac{1}{\Xi}\sum_{N=n}^{\infty} \frac{z^N}{(N-n)!} \int\exp(-\beta W_{N_v}(\gamma_N)) {\rm d} {\vb r}_{n+1} \dotsc {\rm d} {\vb r}_N.
\end{eqnarray*}
...
\begin{equation*}
	\rho^{(1)} ({\vb r}_1) = \Xi^{-1} \sum_{N=1}^{\infty} \frac{z^N}{(N-n)!} \int \exp(-\beta W_{N_v}({\vb r}_1, \dotsc, {\vb r}_N)) {\rm d} {\vb r}_2 \dotsc {\rm d} {\vb r}_N.
\end{equation*}
Applying the method described in~\cite{KKD20}, we obtain the following result
\begin{equation*}
	\rho^{(1)} ({\vb r}_1) = \frac{K_1(\bar{y}, p, \mu)}{v K_0(\bar{y},p,\mu)} = \frac{\bar{n}(p,\mu)}{v}
\end{equation*}
